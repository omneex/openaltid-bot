...
async def initiate_verification(redisClient, member, guild_settings, enabled, bot):
    ...
    if member.created_at <= mintime:
        ...
        try:
            ...
            log.debug(
                f"Verification logs channel: {guild_settings.verification_logs_channel_ID}"
            )
            if guild_settings.verification_logs_channel_ID != 0:
                log.debug("Sending logs to verification channel.")
                channel = bot.get_channel(
                    int(guild_settings.verification_logs_channel_ID)
                )

                embed = discord.Embed(
                    title="Verification Passed!",
                    colour=discord.Colour(0x7ED321),
                    description="The user passed verification.",
                    timestamp=datetime.utcnow(),
                )

                embed.set_author(
                    name="Open/Alt.ID Logs", url="https://github.com/omneex/OpenAltID"
                )
                ...
                embed.add_field(
                    name="Reason",
                    value="__Above minimum age, no verification performed.__",
                )
    ...
    embed = discord.Embed(
        ...
        colour=discord.Colour(0xF0F043),
        description="Verification link has been sent to the user!",
        timestamp=datetime.utcnow(),
    )
    ...
    try:
        while retry:
            if redisClient.get(f"uuid:{unique_ID}") is None:
                redisClient.set(
                    f"uuid:{unique_ID}", f"{member.id}:{member.guild.id}", ex=3600
                )
                retry = False
            else:
                unique_ID = secrets.token_urlsafe(8)
...
class Verification(commands.Cog):
    ...
    async def manual_verify(self, ctx):
        ...
        try:
            ...
            queued = await initiate_verification(
                self.redisClient, ctx.author, guild_settings, True, self.bot
            )
            if queued is True:
            ...
            bot_msg = await ctx.channel.send(
                f"An error occured while queuing verification: {e}"
            )
            await bot_msg.delete(delay=20)
            log.error(e)
    ...
    async def force_verify(self, ctx, user: discord.Member):
        ...
        while True:
            ...
            if msg.content == "done":
                ...
                new_msg = await ctx.bot.wait_for(
                    "message", check=check_yes_no, timeout=60
                )
                if new_msg.content == "yes":
                    ...
                    if result is True:
                        await ctx.channel.send(
                            "Connection added. Add more accounts or type `done` to finish."
                        )
                    else:
                        await ctx.channel.send(f"Failed to add connections: {result}")
                else:
                    await ctx.channel.send(
                        "Connection was not added. Add more accounts or type `done` to finish."
                    )
            ...
            for key in self.redisClient.scan_iter(f"{key_prefix}:*"):
                with self.redisClient.lock("lock:" + key, blocking_timeout=5):
                    ...
                    if value.startswith("true"):
                        ...
                        if guild_settings.verification_role_ID is None:
                            log.error(
                                "The guild does not have a verification role set!"
                            )
                            return
                        ...
                        log.info(
                            f"User: {user_id} was verified in {guild_id}  Score: {score}/{minscore}"
                        )
                        if guild_settings.verification_logs_channel_ID != 0:
                            ...
                            embed.set_author(
                                name="Open/Alt.ID Logs",
                                url="https://github.com/omneex/OpenAltID",
                            )
                            embed.set_footer(text="Powered by Open/Alt.ID")
                            ...
                            embed.add_field(
                                name="Score", value=f"**{score}** / {minscore}"
                            )
                        ...
                        self.redisClient.delete(key)
                        log.info(
                            f"User: {user_id} was NOT verified in {guild_id} Reason: {reason}"
                        )
                        if guild_settings.verification_logs_channel_ID != 0:
                            channel = self.bot.get_channel(
                                int(guild_settings.verification_logs_channel_ID)
                            ...
                            embed = discord.Embed(
                                ...
                                description="The user could not be verified.",
                                timestamp=datetime.utcnow(),
                            )
                        ...
                        self.redisClient.delete(key)
                        log.info(
                            f"User: {user_id} was NOT verified in {guild_id} Score: {score}/{minscore}"
                        )
                        if guild_settings.verification_logs_channel_ID != 0:
                            channel = self.bot.get_channel(
                                int(guild_settings.verification_logs_channel_ID)
                            ...
                            embed = discord.Embed(
                                title="Verification Failed!",
                                colour=discord.Colour(0xD0021B),
                                description="The user did not pass verification.",
                                timestamp=datetime.utcnow(),
                            )
